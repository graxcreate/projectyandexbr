...
def transcribe_audio_speech_recognition(audio_file_path):
    ...
    try:
        transcription = r.recognize_google(audio_data, language="ru-RU")
...
def summarize_texts_individually(texts):
    summarizer = pipeline("summarization", model="UrukHan/t5-russian-summarization")
    summaries = []
    for text in texts:
...
async def process_files(request: TokenRequest):
    ...
    for file_name in mp4_files:
        ...
        try:
            transcription = transcribe_audio_speech_recognition(audio_file_path)
            summary = summarize_texts_individually([transcription])[0]
            results.append({
                "file_name": file_name,
